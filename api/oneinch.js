...
const settleAuction = async (launchId) => {
    try {
        ...
        // Get launch details
        const { data: launch, error: launchError } = await supabaseClient.from("launches").select("*").eq("id", launchId).single();
        ...
        const { data: clearingResult, error: clearingError } = await supabaseClient.rpc("calculate_clearing_price", {
            p_launch_id: launchId,
            p_target_allocation: launch.target_allocation,
        });

        if (clearingError) {
            throw new Error(`Failed to calculate clearing price: ${clearingError.message}`);
        }

        const { clearing_price, filled_quantity, successful_bids_count } = clearingResult[0];
        ...
        // Record settlement
        const { error: settlementError } = await supabaseClient.from("auction_settlements").insert({
            launch_id: launchId,
            clearing_price,
