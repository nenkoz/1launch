...
export const LaunchProvider: React.FC<LaunchProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const loadLaunches = async () => {
      try {
        ...
        const formattedLaunches: Launch[] = launchesData?.map(launch => ({
          id: launch.id,
            ...
            if (!launch.isLaunched && !launch.isSettling) {
              ...
              settleAuction(launch.id).catch(error => {
                ...
                setLaunches(prevLaunches => prevLaunches.map(l =>
                  l.id === launch.id ? { ...l, status: "live" as const, isSettling: false } : l
                ));
              });
  ...
  const submitBid = async (launchId: string, price: number, quantity: number) => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || 'Failed to create bid order');
      }
