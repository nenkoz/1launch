...
const AuctionProtocolArtifact = require('../artifacts/contracts/AuctionProtocol.sol/AuctionProtocol.json');
...
async function main() {
  ...
  const AuctionController = new ethers.ContractFactory(
    AuctionProtocolArtifact.abi,
    AuctionProtocolArtifact.bytecode,
    wallet
  );
  ...
  console.log(`VITE_AUCTION_PROTOCOL_ADDRESS=${auctionControllerAddress}`);
